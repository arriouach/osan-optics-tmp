...
class ZidSaleOrder(models.Model):
        ...
        if connector.auto_create_invoice and sale_order.state == 'sale':
            try:
                ...
                for line in sale_order.order_line:
                    if line.product_uom_qty > 0:  # Only lines with quantity
                ...
                _logger.info(f"Auto-created invoice {invoice.name} for order {sale_order.name}")
                
                # Step 3: Auto-confirm invoice
                if connector.auto_confirm_invoice and invoice.state == 'draft':
                    try:
                        invoice.action_post()
                        _logger.info(f"Auto-confirmed invoice {invoice.name}")
                    except Exception as e:
                        _logger.error(f"Failed to auto-confirm invoice {invoice.name}: {str(e)}")
                        
            except Exception as e:
                _logger.error(f"Failed to auto-create invoice for order {sale_order.name}: {str(e)}")
        ...
        try:
            ...
            for invoice in invoices:
                ...
                existing_payments = self.env['account.payment'].search([
                    ('ref', 'like', f'Zid Order {self.zid_order_id}'),
                ...
                payment_vals = {
                    ...
                    'ref': f'Zid Order {self.zid_order_id} - {self.payment_method_name}',
    ...
    def cron_sync_orders(self):
        """Cron job to sync orders from Zid"""
    ...
    def _create_order_lines(self, sale_order, order_data):
        ...
        mode = self.zid_connector_id.product_match_by
        ...
        for product_data in products_data:
            ...
            # Mode: Create if not found
                ...
                # If not found, create Zid product
                if not odoo_product:
                    ...
                    if not zid_product:
                        _logger.info(f"Zid product {zid_product_id} not found - creating from order data")
                        zid_product = self._create_zid_product_from_order(product_data)
                elif not zid_product and mode == 'always_create':
                    zid_product = self._create_zid_product_from_order(product_data)
                
                if zid_product:
                    # Ensure Zid product is linked to Odoo product
                    if not zid_product.odoo_product_id:
                        _logger.info(f"Linking Zid product {zid_product_id} to Odoo product")
                        zid_product.create_or_update_odoo_product()
                    
                    odoo_product = zid_product.odoo_product_id
            
            if not odoo_product:
                raise UserError(_(f'Could not find or create product for {product_data.get("name")}'))
            ...
            _logger.info(f"Created order line: {odoo_product.name} x {quantity} (mode: {mode})")
    
    def _create_zid_product_from_order(self, product_data):
        """Create minimal Zid product from order line data"""
        zid_product_obj = self.env['zid.product']
        ...
        vals = {
            ...
            'price': float(product_data.get('price', 0)),
            'quantity': 0,  # Don't set quantity from order line
        }
        
        return zid_product_obj.create(vals)
    
    def _create_shipping_line(self, sale_order, order_data):
        """Create shipping line if shipping cost exists"""
        shipping_data = order_data.get('shipping', {})
        
        if not shipping_data:
            return
        
        shipping_cost = float(shipping_data.get('cost', 0) or 0)
        ...
        self.env['sale.order.line'].create(line_vals)
        _logger.info(f"Created shipping line: {shipping_cost}")

    @api.model
    def cron_sync_orders(self):
        """Cron job to sync orders from Zid via proxy"""
        ...
        if not connectors:
            _logger.info("No active Zid connectors found.")
            return
        ...
        for connector in connectors:
            try:
                ...
                # Fetch start date from connector settings
                start_date = connector.order_import_start_date or (fields.Datetime.now() - timedelta(days=7))
                ...
                # Update start date for next sync to NOW
                connector.write({'order_import_start_date': fields.Datetime.now()})
    ...
    def cron_sync_order_status(self):
        """Cron job to sync order status from Zid for recent orders"""
        _logger.info("Starting Zid order status sync cron...")
        
        # Get all active connectors with status sync enabled
        connectors = self.env['zid.connector'].search([
            ('authorization_status', '=', 'connected'),
            ('active', '=', True),
        ...
        if not connectors:
            _logger.info("No active connectors with status sync enabled.")
            return
        ...
        for connector in connectors:
            try:
                _logger.info(f"Syncing order status for connector: {connector.name}")
                ...
                if connector.sync_all_pending_orders:
                    ...
                    _logger.info(f"Checking ALL pending orders for connector {connector.name}")
                else:
                    ...
                    _logger.info(f"Checking recent + pending orders for connector {connector.name}")
                ...
                _logger.info(f"Connector {connector.name}: Checked {checked} orders, updated {updated}")
                
            except Exception as e:
                _logger.error(f"Error syncing order status for connector {connector.name}: {str(e)}")
    ...
    def action_bulk_sync_all_pending(self):
        ...
        for connector in connectors:
            try:
                ...
                if pending_orders:
                    _logger.info(f"Bulk syncing {len(pending_orders)} pending orders for {connector.name}")
                    checked, updated = self._batch_sync_order_status(pending_orders)
                    total_checked += checked
                    total_updated += updated
                
            except Exception as e:
                _logger.error(f"Bulk sync failed for connector {connector.name}: {str(e)}")
