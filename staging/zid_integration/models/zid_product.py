...
class ZidProduct(models.Model):
    ...
    def cron_sync_products(self):
        """Cron job to sync products from Zid via proxy"""
        _logger.info("Starting Zid product sync cron...")
        
        # Get all connected connectors
        connectors = self.env['zid.connector'].search([
            ('authorization_status', '=', 'connected'),
            ('active', '=', True),
            ('product_import_in_progress', '=', False)  # Only process if not already running
        ])
        ...
        for connector in connectors:
            try:
                ...
                connector.write({'product_import_in_progress': True})
                
                _logger.info(f"Syncing products for connector: {connector.app_name}")
                
                # Determine the start date for import:
                # 1. If this is the first import, use product_import_start_date (user configured)
                # 2. If we have imported before, use last_product_import_date for incremental sync
                # 3. Fallback to 30 days ago if nothing is configured
                if connector.last_product_import_date:
                    # Use last import date for incremental sync
                    start_date = connector.last_product_import_date
                    _logger.info(f"Using incremental product sync from last import: {start_date}")
                elif connector.product_import_start_date:
                    # First import - use user configured start date
                    start_date = connector.product_import_start_date
                    _logger.info(f"Using configured start date for first product import: {start_date}")
                else:
                    # Fallback - import last 30 days
                    start_date = fields.Datetime.now() - timedelta(days=30)
                    _logger.info(f"Using fallback start date for products (30 days ago): {start_date}")
                ...
                wizard = self.env['zid.products.connector'].create({
                    ...
                    'update_images': True,  # Always update images in cron
                    ...
                    'update_stock': True,
                    'page_size': 50,
                    'max_pages': 0  # Import all
                })
                
                # Start the import
                wizard.action_import_products()
                
                # Update ONLY the last import date (keep product_import_start_date unchanged)
                connector.write({'last_product_import_date': fields.Datetime.now()})
                
                _logger.info(f"Successfully synced products for connector: {connector.app_name}")
                
            except Exception as e:
                _logger.error(f"Error syncing products for connector {connector.app_name}: {str(e)}")
            finally:
                # Always release the lock
                connector.write({'product_import_in_progress': False})
