...
class StockQuant(models.Model):
    ...
    def create(self, vals_list):
        """Override create to sync new stock to Zid in batch"""
        _logger.info("=" * 80)
        _logger.info(f"[STOCK_QUANT.CREATE] Creating {len(vals_list)} new stock quants")

        quants = super().create(vals_list)

        for quant in quants:
            _logger.info(f"[STOCK_QUANT.CREATE] Quant created: Product={quant.product_id.name}, Qty={quant.quantity}")
            ...
            try:
                ...
                _logger.info(f"[STOCK_QUANT.CREATE] ✅ Zid sync completed successfully for {quant.product_id.name}")
            except Exception as e:
                _logger.error(f"[STOCK_QUANT.CREATE] ❌ Failed to sync to Zid for {quant.product_id.name}: {str(e)}")
                # Don't raise to avoid breaking the stock creation

        _logger.info("[STOCK_QUANT.CREATE] Create batch operation finished")
        _logger.info("=" * 80)
        return quants
