...
class ZidSaleOrderConnector(models.TransientModel):
    ...
    def _prepare_api_params(self):
        ...
        # Date filters
        if self.date_from:
            # Use created_at for new imports, updated_at for sync/updates
            date_attr = 'created_at' if self.import_mode == 'new' else 'updated_at'
            
            if date_attr == 'updated_at':
                params['updated_at_from'] = self.date_from.isoformat()
            else:
                params['created_at_from'] = self.date_from.isoformat()
                
            params['date_from'] = self.date_from.isoformat() # Backward compatibility
            params['date_attribute'] = date_attr
        ...
        if self.date_to:
            ...
            if date_attr == 'updated_at':
                ...
                params['created_at_to'] = self.date_to.isoformat()
    ...
    def action_process_batch(self):
        ...
        try:
            ...
            if self.date_from or self.date_to:
                ...
                for order in orders:
                    date_val_str = order.get(date_attr)
                    if not date_val_str:
                        # Fallback
                        date_val_str = order.get('created_at') or order.get('updated_at')
                    ...
                    try:
                        ...
                        order_date = datetime.strptime(date_val_str, '%Y-%m-%d %H:%M:%S')
                        
                        # Check strictly against creation/update date based on mode
                        ...
                        _logger.warning(f"Could not parse {date_attr} for order {order.get('id')}: {date_val_str}")
                        filtered_orders.append(order)
                
                orders = filtered_orders
                _logger.info(f"Filtered {len(result.get('orders', []))} orders to {len(orders)} within {date_attr} range")
    ...
    def _finish_import(self):
        """Finalize import and return action to show results"""
        self.write({
            'state': 'done',
        ...
        # Return success notification and redirect to Zid Orders
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': _('Import Complete'),
                'message': _('Successfully fetched %d orders and processed the queue.') % self.total_fetched,
                'type': 'success',
                'sticky': False,
                'next': {
                    'name': _('Zid Orders'),
